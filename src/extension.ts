/*
 * extension.ts (and activateXrunDebug.ts) forms the "plugin" that plugs into VS Code and contains the code that
...
import { activateXrunDebug, workspaceFileAccessor } from './activateXrunDebug';
...
export function activate(context: vscode.ExtensionContext) {
	...
	switch (runMode) {
		case 'server':
			...
			activateXrunDebug(context, new XrunDebugAdapterServerDescriptorFactory());
			break;

		case 'namedPipeServer':
			// run the debug adapter as a server inside the extension and communicate via a named pipe (Windows) or UNIX domain socket (non-Windows)
			activateXrunDebug(context, new XrunDebugAdapterNamedPipeServerDescriptorFactory());
			break;

		case 'external': default:
			// run the debug adapter as a separate process
			activateXrunDebug(context, new DebugAdapterExecutableFactory());
			break;
...
class XrunDebugAdapterServerDescriptorFactory implements vscode.DebugAdapterDescriptorFactory {
	...
	createDebugAdapterDescriptor(session: vscode.DebugSession, executable: vscode.DebugAdapterExecutable | undefined): vscode.ProviderResult<vscode.DebugAdapterDescriptor> {
		...
		if (!this.server) {
			...
			this.server = Net.createServer(socket => {
				const session = new XrunDebugSession(workspaceFileAccessor);
...
class XrunDebugAdapterNamedPipeServerDescriptorFactory implements vscode.DebugAdapterDescriptorFactory {
